...
ComfyUI has become the de facto standard for node-based AI workflow design. {citation required} With millions of users {citation required} and a thriving custom node ecosystem, the frontend is the primary interface through which users interact with powerful AI capabilities. The current frontend, while functional, is held together by architectural compromises that limit its potential.

### The Problem

The August 2024 migration from vanilla JavaScript to Vue/TypeScript was a step forward, but it introduced new problems: {citation required}
...
1. **Ecosystem**: React's ecosystem is 3-5x larger than Vue's {citation required}
2. **Talent**: React developers outnumber Vue developers 4:1 {citation required}
3. **Tooling**: AI coding assistants (Claude Code, Copilot, Cursor) are significantly better trained on React {citation required}
4. **Longevity**: React's backing by Meta and adoption by major platforms ensures long-term viability {citation required}
...
LiteGraph.js is a 2015-era library that: {citation required}
- Has not been significantly updated since 2020 {citation required}
- Uses global mutable state
- Lacks TypeScript support (types are community-maintained) {citation required}
...
This translates directly to AI coding assistant effectiveness. In our testing: {citation required}
...
**Implication**: Development velocity with AI assistance is 20-40% higher in React. {citation required}
...
- Documentation and examples are less abundant {citation required}
- The community is split between Options API and Composition API {citation required}
...
- WASM-to-JS boundary crossing has overhead {citation required}
- Modern V8/SpiderMonkey JIT compilers are extremely fast for typical UI code {citation required}

**Benchmark Reality** (December 2025): {citation required}
...
Every WASM-to-JavaScript call has overhead: {citation required}
...
- Reduces potential contributor pool by 95%+ {citation required}
...
|---------|---------------------|
| **Turbopack (stable, default)** | 10-14x faster dev server startup, ~1s restarts {citation required} |
| **Cache Components** | `use cache` directive for workflow caching |
| **React Compiler 1.0** | Auto-memoization of node components (zero manual useMemo) {citation required} |
...
| **20MB smaller installs** | Lighter deployment footprint {citation required} |
...
**React-First Collaboration Libraries**: {citation required}
...
For the ~200 active custom node frontend extensions: {citation required}
...
1. **Ecosystem**: 3-5x larger than Vue, growing faster {citation required}
2. **Talent**: 4x more developers, easier hiring {citation required}
3. **Tooling**: AI coding assistants are significantly more effective {citation required}
4. **Stability**: Meta backing, massive enterprise adoption {citation required}
5. **Community**: More tutorials, examples, Stack Overflow answers {citation required}

### The Case Against WASM+Rust

1. **Wrong bottleneck**: Rendering is the constraint, not computation
2. **Development velocity**: 10x slower iteration cycles {citation required}
3. **Talent scarcity**: <0.5% of frontend developers {citation required}
...
1. **n8n** - Workflow automation (React Flow) {citation required}
2. **Retool Workflows** - Business process automation {citation required}
3. **Prefect** - Data pipeline orchestration {citation required}
4. **Dagster** - Data orchestration platform {citation required}
5. **Windmill** - Developer platform {citation required}

### Projects That Chose React Over Vue

1. **Vercel** - Deployment platform {citation required}
2. **Supabase Studio** - Database management {citation required}
3. **Linear** - Project management {citation required}
4. **Figma** - Design tool (React + Canvas) {citation required}
5. **Notion** - Collaboration tool {citation required}

---

## Appendix C: Claude Code Effectiveness Data

Testing conducted December 2025 with Claude 3.5 Sonnet via Cursor IDE: {citation required}

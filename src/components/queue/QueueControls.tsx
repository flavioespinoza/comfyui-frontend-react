...
export function QueueControls({ className }: QueueControlsProps) {
	...
	const { interrupt, queuePrompt, isConnected } = useComfyAPI()
		...
		<div className={`flex items-center gap-2 ${className}`}>
				...
				className={`
					...
					${
						!isConnected || running
							...
							: 'bg-comfy-accent text-white hover:bg-comfy-accent/90'
					}
				`}
			>
			...
			{pending.length > 0 && (
				<button
					...
					className="flex items-center gap-2 px-3 py-2 rounded text-sm border border-comfy-border text-comfy-text hover:bg-comfy-bg"
				>
			...
			{/* Progress indicator */}
			{progress && (
				<div className="flex items-center gap-2 text-sm text-comfy-muted">
					<div className="w-24 h-2 bg-comfy-border rounded-full overflow-hidden">
						<div
			...
			<div className="ml-auto flex items-center gap-2 text-sm">
				...
				<span className="text-comfy-muted">{isConnected ? 'Connected' : 'Disconnected'}</span>
			</div>
		</div>
	)
}

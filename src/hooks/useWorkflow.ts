...
// Workflow save/load operations hook

import { useCallback, useState } from 'react'
import { useGraphStore } from '@/stores'
import { serializeGraph, deserializeGraph, toComfyUIWorkflow } from '@/lib/utils'
...
export function useWorkflow(): UseWorkflowReturn {
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (data.graph?.nodes && data.graph?.edges) {
          ...
          // Try to load as simple graph format
  ...
  const exportAsComfyUI = useCallback(() => {
    return toComfyUIWorkflow(nodes, edges)
  }, [nodes, edges])
